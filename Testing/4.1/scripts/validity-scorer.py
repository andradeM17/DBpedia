...
def overall_validity():
    with open(f"Testing/4.1/results/{GROUP}-{QUANTITY}/validity of all categories.csv", "w", newline='', encoding="utf-8") as out:
        ...
        for category in categories:
            ...
            with open(f"Testing/4.1/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv", newline='', encoding="utf-8") as f:
                reader = csv.reader(f)
                for row in reader:
                    validity = row[3]
...
def fine_grained_validity():
    with open(f"Testing/4.1/results/{GROUP}-{QUANTITY}/validity of all entities.csv", "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
        ...
        for category in categories:
            with open(f"Testing/4.1/output/pages/{GROUP}-{QUANTITY}/{category}.txt", encoding="utf-8") as entities_file:
...
def invalid_entity_types():
    ...
    for category in categories:
        with open(f"Testing/4.1/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv",
    ...
    with open(f"Testing/4.3/{GROUP}-{QUANTITY}/invalid-types-pairs.csv",
...
def invalid_properties():
    ...
    with open(f"Testing/4.4/{GROUP}-{QUANTITY}/invalid-properties.csv",
              "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
...
def not_fully_valid_properties():
    counts = defaultdict(lambda: defaultdict(int))
    for category in categories:
        with open(f"Testing/4.1/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv",
                  newline='', encoding="utf-8") as f:
            ...
            not_fully_valid_statuses = {
                "Valid (No expected domain, no expected range)",
                "Valid (No expected domain, range matches)",
                "Valid (Domain matches, no expected range)",
            }

            for row in reader:
                if row[3] in not_fully_valid_statuses:
                    prop = row[1]
                    counts[prop][category] += 1

    with open(f"Testing/4.4/{GROUP}-{QUANTITY}/not-fully-valid-properties.csv", "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)

        # Calculate total counts for each property
        property_totals = {
            prop: sum(counts[prop][category] for category in categories)
            for prop in counts
        }

        # Overall total
        grand_total = sum(property_totals.values())

        # Header
        writer.writerow(["Property", "Total", "Percentage"] + categories)

        # Sort properties by descending total
        sorted_props = sorted(
            counts.keys(),
            key=lambda prop: property_totals[prop],
            reverse=True
        )

        # Write rows
        for prop in sorted_props:
            total = property_totals[prop]
            percentage = (100 * total / grand_total) if grand_total else 0
            ...
            row.extend(counts[prop][category] for category in categories)
...
def all_properties():
    ...
    # Write pivot table
    with open(f"Testing/4.1/results/{GROUP}-{QUANTITY}/all-properties.csv",
              "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
        # Header row
        writer.writerow(["Property"] + categories)
