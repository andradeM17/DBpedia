...
def get_values(category, titles):
    ...
    """
    with open(f"Testing/4.5/output/values/{category}-values.csv", "w", newline="", encoding="utf-8") as f:
        new_titles = []
...
def validate(category):
    ...
    with open(f"Testing/4.5/output/values/{category}-values.csv", newline="", encoding="utf-8") as values, \
    open(f"Testing/4.5/output/validations/{category}-validations.csv", "w", newline="", encoding="utf-8") as validations:
...
def get_du_values(category):
    with open(f"Testing/4.5/output/values/{category}-values.csv", newline="", encoding="utf-8") as values, open(f"Testing/4.5/output/pages/{category}.txt", newline="", encoding="utf-8") as pages:
        reader = csv.reader(values)
                    ...
                    for direction, result in results:
                        ...
                        for binding in result["results"]["bindings"]:
                            ...
                            if LIMIT == -1 or prop_count.get(prop, 0) < LIMIT:
                                ...
                                prop_count[prop] = prop_count.get(prop, 0) + 1
    with open(f"Testing/4.5/output/values/{category}-values.csv", "a", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["Subject", "Property", "Object"])
...
def get_pages_from_specific_list():
    ...
    with open(f"Testing/4.5/output/pages/{category}.txt", "w", encoding="utf-8") as f:
        for binding in result['results']['bindings']:
    ...
    if choice == "E":
        ...
        for category in CATEGORIES:
            ...
            if current_category:
                ...
                with open(f"Testing/4.5/output/pages/{current_category}.txt", "r", encoding="utf-8") as f:
                ...
                with open(f"Testing/4.5/output/pages/{current_category}.txt", "w", encoding="utf-8") as f:
