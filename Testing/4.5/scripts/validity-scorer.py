...
def overall_validity():
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/validity of all categories.csv", "w", newline='', encoding="utf-8") as out:
        ...
        for category in categories:
            ...
            with open(f"Testing/4.5/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv", newline='', encoding="utf-8") as f:
                reader = csv.reader(f)
                for row in reader:
                    validity = row[3]
...
def fine_grained_validity():
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/validity of all entities.csv", "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
        ...
        for category in categories:
            with open(f"Testing/4.5/output/pages/{GROUP}-{QUANTITY}/{category}.txt", encoding="utf-8") as entities_file:
...
def invalid_entity_types():
    ...
    for category in categories:
        with open(f"Testing/4.5/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv",
    ...
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/invalid-types-pairs.csv",
...
def invalid_properties():
    ...
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/invalid-properties.csv",
              "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
...
def all_properties():
    ...
    # Write pivot table
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/all-properties.csv",
              "w", newline='', encoding="utf-8") as g:
        writer = csv.writer(g)
        # Header row
        writer.writerow(["Property"] + categories)
...
def all_valid_triples():
    with open(f"Testing/4.5/results/{GROUP}-{QUANTITY}/all-valid-triples.csv", "w", newline='', encoding="utf-8") as out:
        writer = csv.writer(out)
        writer.writerow(["Subject", "Property", "Object", "Validity"])
        ...
        for category in categories:
            with open(f"Testing/4.5/output/validations/{GROUP}-{QUANTITY}/{category}-validations.csv",
                ...
                for row in reader:
                    validity = row[3]
                    if validity.startswith("Valid"):
                        writer.writerow(row[:4])  # Write subject, property, object, validity
