...
#SECTION = "Science"  # Change this to switch sections
...
class DBpediaValidator:
    ...
    def clean_name(self, name):
        ...
        if new_name != name:
            print(f"Cleaned '{name}' to '{new_name}'")
        return new_name
...
def main():
    ...
    list_of_topics = ["Food and agriculture", "Geography", "History", "Languages and literature", "Measurements", "Philosophy", "Religion", "Science", "Social sciences", "Technology"]
    
    while list_of_topics:
        SECTION = list_of_topics.pop(0)

        input_file = f'Testing/1.4/dbo-rk/files-sorted.csv'
        output_file = f'Testing/1.4/dbp-rk/Validations/{SECTION}_validations.csv'
        invalid_file = f'Testing/1.4/dbp-rk/Fails/{SECTION}_invalid_instances.csv'
