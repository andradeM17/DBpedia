...
SECTION = "test"  # Change this to switch sections
    ...
    def get_all_outgoing_properties(self, page_name):
        ...
        SELECT ?property ?value WHERE {{
            ...
            FILTER(?property NOT IN (
                ...
                dbo:damage, dbo:depictionDescription, dbo:event, dbo:imageSize, dbo:impactFactorAsOf, dbo:isHandicappedAccessible,
...
def main():
    ...
    with open(f'Testing/1.3/{SECTION}_validations.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
    ...
    with open(f'Testing/1.3/Validations/{SECTION}_validations.csv', 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Page Name", "Score", "Datapoints", "Properties Score", "Properties Checked", "Incoming Score", "Incoming Checked"])
        for page in pages:
            score, datapoints, propscore, proptotal, isofscore, isoftotal = validator.validate_resource(page)
            writer.writerow([page, score, datapoints, propscore, proptotal, isofscore, isoftotal])
            f.flush()
            print(f"Written {page} to CSV")
            print("=" * 100)


    # Save invalid instances to CSV
    invalid_filename = f'Testing/1.3/Fails/{SECTION}_invalid_instances.csv'
    validator.save_invalid_instances(invalid_filename)
